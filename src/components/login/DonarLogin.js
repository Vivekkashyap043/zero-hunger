import { useForm } from "react-hook-form";
import {useState} from 'react'
import './DonarLogin.css'
import {useNavigate} from 'react-router-dom'


function DonarLogin() {
  let [users,setUsers]=useState([])
  let [res, setRes]=useState({})
  const navigate = useNavigate()


  let { register, handleSubmit, formState: { errors }} = useForm();

 async function onDonarLogin(donarObj) {
    try {
      const response = await fetch("http://localhost:4000/donar/login", {
        method: "POST",
        body: JSON.stringify(donarObj),
        headers: {
          'Content-Type': 'application/json'
        }
      });
      if (!response.ok) {
        throw new Error('Network response was not ok.');
      }
      const data = await response.json();
      setRes(data);
      console.log(data); // Logging data instead of res
      if(data.status==="success"){
        navigate(`/donar-dashboard/${donarObj.username}`, {state: donarObj})
      }
    } catch (error) {
      console.error('Error:', error);
    }
  }
  return (
    <div>
      <form onSubmit={handleSubmit(onDonarLogin)}>
        <div class="main-custom">
          <div class="left">
            <div class="up">
              <h1 className="donate">Donor</h1>
              <p className="fs-5">Login here</p>
              <div class="icons">
                <a href="/auth/google" target="_blank"><img src="data:image/png;base64," alt="" /></a>
              </div>
              <p class="acc">⎯⎯⎯⎯ or use your account ⎯⎯⎯⎯</p>
            </div>
            <div>
              <div >
                <input type="text" className="in1" placeholder="Enter your username" {...register("username", { required: true })}/>
              </div>
              
            </div>
            <div >
              <input type="password" className="in2" placeholder="Enter your password" {...register("password", { required: true })}/>
            </div>
            {res.status==="notexist" && (
                      <p className="text-danger text-center m-0">Invalid username</p>
              )}
              {res.status==="passwordnotmatch" && (
                      <p className="text-danger text-center m-0">Incorrect password</p>
              )}
            <button type="submit" className="fs-5">Login</button>
          </div>
        </div>
      </form>
    </div>
  )
  }
    
export default DonarLogin;